...
# Write per-call CSV with system prompt info
...
with open(call_csv_file, 'w', newline='') as f:
    ...
    writer.writerow([
        ...
        'System Chars', 'System Tokens (est)', 'User Prompt (truncated)'
    ])
...
with open(output_file, 'w', newline='') as f:
    ...
    # Add system prompt category columns
    category_headers = sorted(category_tokens.keys())
    header = [
    ...
    ] + category_headers + ['User Prompt (truncated)']
    ...
    for turn_num in sorted(turns.keys()):
        ...
        row = [
            ...
            system_prompt_chars,
            system_tokens_est,
        ] + [category_tokens.get(cat, 0) for cat in category_headers] + [
