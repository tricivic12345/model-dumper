...
session_messages = []
session_data_timestamps = []  # Store data.timestamp for each message
with open(session_file, 'r') as f:
    for line in f:
        try:
            data = json.loads(line.strip())
            if data.get('type') == 'message':
                msg = data.get('message', {})
                # Store data.timestamp as ISO string
                msg['_raw_timestamp'] = data.get('timestamp', '')
                # Parse and store as epoch ms for timing
...
for i, intercept_rec in enumerate(intercept_entries):
    # Get timestamp - intercept uses epoch milliseconds
    intercept_ts = intercept_rec.get('timestamp', 0)
    
    # Handle both epoch ms (int) and ISO string formats
    if isinstance(intercept_ts, int):
        # Already epoch milliseconds
        ts_epoch = intercept_ts
        ts_human = datetime.fromtimestamp(intercept_ts / 1000).isoformat() if intercept_ts > 0 else ""
    elif isinstance(intercept_ts, str) and intercept_ts:
        try:
            dt = datetime.fromisoformat(intercept_ts.replace('Z', '+00:00'))
            ts_epoch = int(dt.timestamp() * 1000)
    ...
    if i in matches:
        ...
        # Get the user message that triggered this request
        # Use _event_epoch (epoch ms) for accurate timing
        user_msg_idx = session_idx - 1 if session_idx > 0 else None
        user_timestamp = 0
        if user_msg_idx is not None and session_messages[user_msg_idx].get('role') == 'user':
            user_timestamp = session_messages[user_msg_idx].get('_event_epoch', 0)
        ...
        # Use _event_epoch for assistant (epoch ms from data.timestamp)
        assistant_timestamp = session_msg.get('_event_epoch', 0)
        
        total_time = 0
        tpot = 0
        timing_note = None
        
        if user_timestamp > 0 and assistant_timestamp > 0:
            # Both timestamps are in epoch milliseconds
            total_time = assistant_timestamp - user_timestamp
            # TPOT: Time Per Output Token (ms/token)
            if output_tokens > 0:
                tpot = total_time / output_tokens
            timing_note = "Calculated from user message timestamp and assistant message timestamp"
        else:
            timing_note = "OpenRouter does not expose TTFT, TPOT. prefill/decode not available."
    else:
        ...
        for msg in messages:
            if msg.get('role') == 'assistant':
                ...
                timing_note = "Calculated from intercept (fallback)"
                break
        if not timing_note:
            timing_note = "OpenRouter does not expose TTFT, TPOT. prefill/decode not available."
    ...
    entry = {
        ...
        "timing": {
            "total_time_ms": total_time if total_time > 0 else None,
            "tpot_ms": round(tpot, 2) if tpot > 0 else None,
            "ttft_ms": None,  # OpenRouter does not expose TTFT
            "prefill_ms": None,  # OpenRouter does not expose prefill time
            "decode_ms": None,  # OpenRouter does not expose decode time
        },
        "_timing_note": timing_note,
        "_input_chars": input_len,
        "_matched": i in matches,
    }
    entries.append(entry)
...
if output_file:
    with open(output_file, 'w') as f:
        for entry in entries:
            ...
            for key in ['_input_chars', '_matched', '_timing_note']:
                if key in entry:
                    del entry[key]
            f.write(json.dumps(entry, ensure_ascii=False) + '\n')
    print(f"Exported {len(entries)} entries to {output_file}")
        ...
        try:
            ...
            if data.get('type') == 'message':
                ...
                if role == 'user':
                    ...
                    # Calculate call duration and TPOT from previous message
    call_duration = 0
    tpot = 0
    if prev_timestamp:
        ...
        if curr_ts and prev_ts:
            ...
            if output_tok > 0:
                tpot = (call_duration * 1000) / output_tok
...
with open(call_csv_file, 'w', newline='') as f:
    ...
    # Header with system prompt breakdown columns and timing
    header = [
        'Call#', 'Turn', 'Input Tokens', 'Output Tokens', 'Total Tokens', 
        'Cache Read', 'Cache Write', 'Tool Calls', 'Stop Reason',
        'Call Duration (s)', 'TPOT (ms/token)',
        'TTFT (ms)', 'Prefill (ms)', 'Decode (ms)', 'Note',
        'System Chars', 'System Tokens (est)',
    ] + system_columns + ['User Prompt (truncated)']
    writer.writerow(header)
    
    # Note: OpenRouter does not expose TTFT, prefill, decode times
    # TPOT is calculated: (call_duration * 1000) / output_tokens
    timing_note = 'N/A: OpenRouter does not expose TTFT/Prefill/Decode'
    ...
    for call in api_calls:
        row = [
            ...
            f"{call['tpot']:.2f}" if call['tpot'] > 0 else '-',
            '-',  # TTFT - not available from OpenRouter
            '-',  # Prefill - not available from OpenRouter
            '-',  # Decode - not available from OpenRouter
            timing_note,
            system_prompt_chars,
            system_tokens_est,
...
with open(output_file, 'w', newline='') as f:
    ...
    for turn_num in sorted(turns.keys()):
        r = turns[turn_num]
        growth = r['last_input'] - r['first_input']
        # Calculate average TPOT for this turn
        turn_calls = [c for c in api_calls if c['user_turn'] == turn_num]
        avg_tpot = sum(c['tpot'] for c in turn_calls) / len(turn_calls) if turn_calls else 0
        row = [
            turn_num,
            r['api_calls'],
            r['input_tokens'],
            ...
            f"{avg_tpot:.2f}" if avg_tpot > 0 else '-',
            system_prompt_chars,
            system_tokens_est,
        ] + [category_tokens.get(col, 0) for col in system_columns] + [
