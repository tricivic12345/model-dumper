...
entries = []
current_input_parts = []  # List of input parts to join
...
for i, msg in enumerate(messages):
    ...
    if role == 'user':
        # New user message - add system prompt first
        if not any(p.startswith('[SYSTEM]') for p in current_input_parts) and system_prompt:
            current_input_parts.append(f"[SYSTEM]\n{system_prompt}")
        
        content = msg.get('content', [])
        text = extract_text_content(content)
        current_input_parts.append(f"[USER]\n{text}")
        
    elif role == 'toolResult':
        content = msg.get('content', [])
        text = extract_text_content(content)
        tool_name = msg.get('toolName', 'unknown')
        current_input_parts.append(f"[TOOL_RESULT: {tool_name}]\n{text}")
        ...
        if current_input_parts:
            ...
            # Build output with thinking, tool calls, and text
            output_parts = []
            if thinking:
            ...
            # Add assistant response to input for next call
            assistant_part = f"[ASSISTANT]\n{output_text}"
            if thinking:
                assistant_part = f"[ASSISTANT thinking]\n{thinking}\n\n{assistant_part}"
            current_input_parts.append(assistant_part)
